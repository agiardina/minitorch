class Module:
    ...
    def named_parameters(self):
        ...
        """
        ls = []
        for key in self.__dict__["_parameters"].keys():
            ls.append((key,self._parameters[key]))

        for key in self.__dict__["_modules"].keys():
            sub_ls = self._modules[key].named_parameters()
            ls += map(lambda x: (key+"."+x[0],x[1]),sub_ls)
